...
- run `cargo lambda build --output-format=zip`, and then output is in target/lambda/test-lambda-fn/bootstrap.zip.
- that bootstrap.zip file can be copied into /lambda and then you can run `cargo run ./bootstrap.zip myfunc mypath`

# Introduction

test-lambda-fn is a Rust project that implements an AWS Lambda function in Rust.
